...
(function() {
  ...
  var ROOMS = [
    ...
    [xBR,   zT,    xHall, zM1,  '#c8a870', '침실 1',   9.15, 1.35],   // @ROOM#7  침실1
  ...
  var WALLS = [
    ...
    [xBR,    zT,   xHall, zT  ],   // 침실1(7번 방) 위쪽 외벽
  ...
  var DOORS = [
    ...
    [9.01,  3.77,   '신발장 문(좌)'],     // @DOOR#47 _doors[31]  inline 2 (양문형 좌)
    [9.57,  3.77,   '신발장 문(우)'],     // @DOOR#48 _doors[32]  inline 2 (양문형 우)
  ];

  // 가구 위치 (참조용 번호 부여) [x, z, label]
  // 안정 식별자(@FURN#NN): 글로벌 번호 = 15(ROOMS) + 31(DOORS) + i + 1.
  //   i=0  → @FURN#47, i=26 → @FURN#73. 코드 안 IIFE 첫 줄에도 같은 앵커 부착.
  var FURNITURE = [
    [0.50,  6.09, '드럼세탁기'],          // @FURN#47 발코니 좌측, 0.9×0.9×1.275 m
    [3.30,  5.52, '침실2 침대'],          // @FURN#48 퀸, 1.65×2.05×0.22(프레임)
    [10.04, 1.35, '침실1 책상'],          // @FURN#49 동측 벽 부착, 1.80×0.80×0.74
    [9.57,  3.77, '현관 신발장'],         // @FURN#50 양문형 좌측 2칸 활성, 우측 1칸은 기둥
    ...
    [6.54,  0.22, '주방 상부장(앞)'],     // @FURN#62 y=1.55~2.40
    [0.55,  1.80, '실내 자전거'],         // @FURN#63 거실확장 NW 코너 (Tacx Neo Bike Smart 스타일)
    [9.84,  0.185,'침실1 책꽂이 1'],      // @FURN#64 침실1 NE 코너, 폭 1.20 m, 5단 오픈 (북측 우)
    [8.64,  0.185,'침실1 책꽂이 2'],      // @FURN#65 책꽂이 64 좌측 인접, 폭 1.20 m (북측 좌)
    [9.64,  2.515,'침실1 책꽂이 3'],      // @FURN#66 책꽂이 64 의 z 미러 (남측), 폭 1.60 m
  ...
  var WINDOWS = [
    ...
    [10.5, 1.41, '침실1-복도 창'],         // @WIN#121 폭 1.35 m
  ...
  var FURNITURE_BBOX = [
    ...
    [9.64,  0.45,  10.44, 2.25],            // 29 침실1 책상
    [8.7,   3.77,  9.88,  4.14],            // 30 현관 신발장 (좌측 2칸만, 우측 1칸은 방 15 기둥)
    ...
    [5.34,  0.06,  7.74,  0.38, 1.5,  2.4 ],// 42 주방 상부장(앞) — y 1.5 이상
    [0.20,  1.05,  0.90,  2.55, 0,    1.60],// 47 실내 자전거 (디테일 강화)
    [9.24,  0.06,  10.44, 0.31, 0,    1.85],// @FURN#64 침실1 책꽂이 1 (NE 코너, 벽 59 등+벽 74 우측면, 폭 1.20m)
    [8.04,  0.06,  9.24,  0.31, 0,    1.85],// @FURN#65 침실1 책꽂이 2 (책꽂이1 좌측 인접, 폭 1.20m)
    [8.84,  2.39,  10.44, 2.64, 0,    1.85],// @FURN#66 침실1 책꽂이 3 (책꽂이1 의 z 미러, 남측 벽, 폭 1.60m)
  ...
  var WINDOWS_BBOX = [
    ...
    [10.40, 0.735, 10.60, 2.085], // 침실1-복도 창 (1.5x 확장)
