...
const OUTLETS = [
  ...
  // ── 서재 (방 7) ───────────────────────────────────────────────
  // 사용자 요청 (본 CL): 서재 안에서 벽 98 (서재 동측 외벽 x=10.5) 바라볼 때
  //   보이는 콘센트 2개 (face='W', xHall-WT/2=10.44 의 z=0.50 + z=2.20) 모두 제거.
  // (이 위치엔 outlet 없음 — 본 CL 에서 두 outlet 제거 후 wall 98 서재 측 비어있음.)
  ...
  // 사용자 요청 (본 CL): 벽 104 (현 badge, [xBR+DW, zM1, xHall, zM1] = z=2.7,
  //   x=8.7~10.5 = 서재 북측 외벽) 에 콘센트 1개 추가. 벽 98 서재 측 outlet
  //   2개 제거 (위) 보상 — 서재 측 (face='S') 2구.
  { x: 9.60,  y: 0.40, z: zM1 - WT/2,      face: 'S', gangs: 2, label: '서재 북측 (벽 104)' },
  // ── 복도 (방 8) ─────────────────────────────────────────────────
