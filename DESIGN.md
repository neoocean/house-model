...
### 4.31. 벽 103 컨트롤 패널 위치 이전 — 서편 → 동편 (본 CL)

**보고**: "앞서 설치한 인터폰, 스위치, 온도조절기는 현재 거실에서 벽 103을 바라볼 때 벽 오른편에 있습니다. 이들을 벽 왼편으로 옮겨주세요."

**원인 분석**: 직전 CL 51057 에서 cluster center `cxBase=1.70` (벽 103 의 서쪽 끝 x=1.2 에서 50 cm 우) 에 배치. 거실에서 1인칭으로 벽 103 (z=3.5, +z 방향) 을 바라볼 때 카메라 yaw=π. Three.js 카메라 right 벡터 = `(cos(yaw), 0, -sin(yaw))` 이라 yaw=π 일 때 right = (-1, 0, 0) = -x. 따라서 작은 x = 사용자 시야의 RIGHT, 큰 x = 사용자 시야의 LEFT.

**변경**:
- `cxBase`: 1.70 → **4.90** (벽 동쪽 끝 xKit=5.1 에서 20 cm 서쪽, 침실 2 우측 벽 코너 근처)
- 인터폰 / 스위치 / 온도조절기 모두 cxBase 기반 상대 배치 (`cxBase + offset`) 라 한 줄 수정으로 자동 이동
- y, z, 두께, 단면 등 다른 모든 dimension 불변
...
| 인터폰 | (4.90, 1.45, 3.42) |
| 스위치 | (4.82, 1.10, 3.434) |
| 온도조절기 | (4.98, 1.10, 3.4275) |

**교훈** (1인칭 좌·우 인지의 yaw 의존성):

1인칭 모드에서 사용자가 "벽 N 의 왼편/오른편" 이라 할 때 — **실 좌표 x 방향과 화면 left/right 의 매핑은 카메라 facing (yaw) 에 따라 반전**.

| yaw | 방향 | 카메라 right | 화면 right |
|---|---|---|---|
| 0 | -z 남향 | +x | +x (동) |
| π/2 | +x 동향 | +z | +z (북) |
| π | +z 북향 | -x | -x (서) |
...
벽 103 은 거실의 북쪽 경계 (z=3.5). 거실 (z<3.5) 에서 보려면 yaw=π (북향). 이때 작은 x = 화면 right, 큰 x = 화면 left.

따라서 사용자가 "벽 103 왼편" 이라 하면 = 카메라 left = 큰 x = 동쪽 끝 (x near 5.1). 직관적인 "서쪽 = 왼쪽" 통상적 매핑은 yaw=0 일 때만 성립.

**향후 좌·우 명시 시**: 사용자가 어느 벽을 어느 시야 (yaw) 에서 보는지 추측 → 카메라 right 벡터 계산 → 매핑. 사진/그림 첨부 시 편 추정이 더 안전.

### 4.32. 벽 103 컨트롤 — 스위치 ↔ 온도조절기 swap (본 CL)

**요청**: "벽 103의 스위치와 온도조절기의 위치를 서로 바꿔주세요."

**변경 (offset 부호만 swap)**:
| 부재 | 직전 (CL 51063) | 본 CL |
|---|---|---|
| 스위치 swX | cxBase - 0.08 = 4.82 (시야 RIGHT) | **cxBase + 0.08 = 4.98** (시야 LEFT) |
| 온도조절기 thX | cxBase + 0.08 = 4.98 (시야 LEFT) | **cxBase - 0.08 = 4.82** (시야 RIGHT) |

cxBase=4.90 불변. 인터폰 (cxBase 위치) 도 불변.

**의도**: 사진 (CL 51057 첨부) 의 좌·우 배치 = 스위치 LEFT, 온도조절기 RIGHT. 이전 배치는 사진과 반대였음 — 본 CL 로 사진 일치.

**참고**: §4.31 의 1인칭 yaw=π 매핑 (북향 카메라 right=-x → 작은 x=화면 right, 큰 x=화면 left). 사용자 시야 기준 LEFT/RIGHT 의도라면 +offset = LEFT, -offset = RIGHT.

### 4.33. 서재·주방 천장 조명 정비 + 서재 콘센트 정리 (본 CL)

**요청 (한 메시지 6 sub-task)**:
1. 서재 천장 조명 모두 제거 → 4 모서리 각 2개씩 (8개) 신설
2. 서재 안에서 벽 98 (현 badge, 서재 동측 외벽 x=10.5) 바라볼 때 보이는 콘센트 2개 모두 제거
3. 벽 104 (현 badge, 서재 북측 외벽) 에 콘센트 1개 추가
4. 주방에서 벽 95 (주방-서재 경계 x=7.8) 바라볼 때 양쪽 천장에 각 2개씩 (4개) 신설
5. 벽 82 (주방·식당 남측 외벽 z=0) 좌측 모서리 + 천장 만나는 지점에 1개 신설
6. 벽 117 (거실↔주방 배관 기둥 남측 z=0.75) 천장 만나는 지점에 1개 신설

모든 신규 천장 조명: **지름 2 인치 (size:0.05)**, kind:'flush'.

**신규 천장 조명 (14 개)**:
| 그룹 | 위치 |
|---|---|
| 서재 모서리 NW pair | (8.00, 0.20), (8.30, 0.20) |
| 서재 모서리 NE pair | (10.00, 0.20), (10.30, 0.20) |
| 서재 모서리 SW pair | (8.00, 2.50), (8.30, 2.50) |
| 서재 모서리 SE pair | (10.00, 2.50), (10.30, 2.50) |
| 주방 벽95 천장 | (7.50, 0.60), (7.50, 2.00) |
| 서재 벽95 천장 | (8.10, 0.60), (8.10, 2.00) |
| 주방 벽82 좌측 | (5.30, 0.20) |
| 거실 벽117 | (5.10, 0.78) |

서재 측 벽 95 (x=8.10) + 모서리 NW/SW (x=8.00~8.30) 의 z 분리 (코너 0.20/2.50, 미들 0.60/2.00) 로 시각 중복 회피.

**제거된 조명**: '서재 ①' (CL 51059, x=9.15, z=0.95), '서재 ②' (x=9.15, z=1.75).

**OUTLETS 변경**:
- 제거: '서재 책상 좌' (xHall-WT/2=10.44, 0.40, 0.50, face='W'), '서재 남쪽' (10.44, 0.40, 2.20, face='W') — 둘 다 벽 98 서재 측.
- 추가: '서재 북측 (벽 104)' (9.60, 0.40, zM1-WT/2=2.64, face='S', gangs=2).

**왜 face='S'**: 벽 104 = z=2.7 수평 벽. face='S' = -z 방향 (서재 안쪽 향). 서재가 벽 98 측 outlet 2개 잃었으므로 서재 측에 보상.

**가시성 검증** (사용자가 이전에 강조: "1번 모드에서도 보여야"):
- 천장 조명: `_initPowerPlanCache` 의 hide 분류는 FURN bbox 안 메시만 대상. 서재 가구 (책상 @FURN#49, 책꽂이 @FURN#64/65/66, 자전거 @FURN#67) 의 어떤 bbox 도 새 천장 등 위치 (x=8.00~10.30, z=0.20~2.50, y≈2.39) 와 매치 안 됨 → 자동 가시 ✓
- 주방·거실 새 등 (5.10~7.50): 거실 가구 (소파 @FURN#70 [1.56~2.41], 다이닝 [2.80~4.40]) / 주방 가구 (하부장 [5.34~7.74]) 의 bbox 와 천장 등 위치 (y≈2.39, 가구 y≤1.0) 의 y 차이로 not 매치 → 자동 가시 ✓

**[CC] 어셔션**: FURN_META 변경 없음 → 영향 0.

**Stale label 노트**: 기존 outlets.js 에 "벽 82" / "벽 104" 등 라벨이 있는데 이는 가구 추가 누적으로 시프트 전 badge — 본 CL 의 추가 outlet '서재 북측 (벽 104)' 도 미래 가구 추가 시 시프트 가능. 안정 식별자는 좌표 (`x=xBR+DW~xHall, z=zM1`).

**5/8 미팅 ③ 조명 진행도** (본 CL 후): 서재 + 주방 + 거실 천장 조명 대폭 추가 — 미팅 결정 ③ "조명 위치 및 수량" 의 조명 부분 진척. 다른 방 조명은 계속 진행 중.
