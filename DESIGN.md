...
**현관 ↔ 주방·식당 중문 (영림 초슬림 간살 3연동)** — 사용자 요청에 따라 신발장 정면 우측 빈 공간 (cabinet-relative -x, x=7.8 라인) 의 1.5 m 개구 (z=2.7~4.2) 에 3 패널 슬라이딩 vestibule door 설치. 각 패널 0.50 m × 2.30 m × 22 mm, 3 트랙 시스템 (x=7.770/7.800/7.830, ±15 mm 간격). 알루미늄 다크 차콜 프레임 (좌우 30 mm / 상하 40 mm) + 프로스티드 글래스 (opacity 0.32) + 세로 간살 5 살/패널 (8 × 8 mm 단면, 양면 부착). 상부 천장 직하 박스 레일 (100 × 60 mm) + 하부 바닥 라인 가이드. **인터랙티브 미닫이** — 사용자 요청에 따라 클릭 시 3 패널이 함께 +z (북쪽) 방향으로 슬라이드: 패널 1 (+1.0 m), 패널 2 (+0.5 m), 패널 3 (정지) → 모두 z=3.7~4.2 에 적층, 남쪽 z=2.7~3.7 (1 m) 통로 개방. `_doors[]` 에 `kind:'slide'` + `linkGroup:'jungmun'` 으로 등록 — 어느 패널 클릭해도 3 개 모두 동기 토글. @DOOR#49/50/51 (이전 33 종 → 36 종, FURN/WALL/WIN 배지 번호 +3 시프트 — `벽 96`→`벽 99`, `가구 49`→`가구 52` 등). 인접 입구 (침실1/창고) 와 미충돌.
