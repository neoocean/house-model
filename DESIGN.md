...
**현관 ↔ 주방·식당 중문 (영림 초슬림 간살 3연동)** — 사용자 요청에 따라 신발장 정면 우측 빈 공간 (cabinet-relative -x, x=7.8 라인) 의 1.5 m 개구 (z=2.7~4.2) 에 3 패널 슬라이딩 vestibule door 설치. 각 패널 0.50 m × 2.30 m × 22 mm, 3 트랙 시스템 (x=7.770/7.800/7.830, ±15 mm 간격). 알루미늄 다크 차콜 프레임 (좌우 30 mm / 상하 40 mm) + 프로스티드 글래스 (opacity 0.32) + 세로 간살 5 살/패널 (8 × 8 mm 단면, 양면 부착). 상부 천장 직하 박스 레일 (100 × 60 mm) + 하부 바닥 라인 가이드. 현재 **닫힘 정적** 상태 — 슬라이딩 애니메이션 미구현 (추후 추가 가능). @FURN#/@DOOR# 미등록 — 인접 입구 (침실1/창고) 와 미충돌, 모서리 점 접촉만.
