...
| 연결통로 2 / 현관 | 7.8 – 10.5 | 2.7 – 4.2 | **녹색 슬라브, 5 cm 단차** (신발 영역) — 서측은 **중문 동면 x=8.896 까지** (CL 50302). 중문 서측 (x<8.896) 은 타일 바닥. 신발장 정면 앞 (z=2.7~3.77) 에 **중문 디딤판** — 동측으로 +30 cm (x=8.896~9.196) 돌출 (CL 50490+, 사용자 요청). 좌측 양문형 신발장. |
...
- **욕실 바닥** (`mBath`, 청회색 단색).
- **발코니 바닥** (`mBal`, 베이지 단색).
- **현관 바닥** (`mEntry`, 녹색, -5 cm 단차). 기본 x 8.896~10.5 / z 2.7~4.0 의 직사각형이었으나 **중문 디딤판 신설** (CL 50490+, 사용자 요청) 로 L-shape: (a) 디딤판 동측 x=9.196~10.5, z=2.7~3.77, (b) 디딤판 북측 x=8.896~10.5, z=3.77~4.0. 디딤판 자체 (x=8.896~9.196, z=2.7~3.77) 는 buildTileFloor 의 quad 로 채워져 5 cm 단차 위 — 신발장 정면 발 디딤 공간 제공. 중문 서측 (주방·식당 측) 에서는 녹색 안 보임.
- **천장** (`mCeil`, 1인칭 전용 토글).
...
**후속 (CL 50490+)** — 신발장 정면 앞 **중문 디딤판** (사용자 요청, "신발장 방향에서 볼 때 중문 아래쪽 바닥이 신발장 앞쪽으로 30 cm 더 튀어나오도록"). 중문 동면 (x=8.896) 에서 동측으로 +30 cm (→9.196), 신발장 (z=3.77~4.14) 남측 z=2.7~3.77 구간에 한정. 영향:
