...
- **정적 레이어 캐싱** (항목 D): 방 채움(15)·벽 라인(43)·방 라벨(8)·타이틀·번호 배지(120+) 를 두 개의 오프스크린 캔버스(`_staticBgCv` / `_staticTopCv`) 에 1 회만 렌더하고, 매 프레임 `drawImage` 두 번 + 동적 FOV 부채꼴/포워드 레이/카메라 점만 그림. 200+ 그리기 호출 → 5~6 회로 감소.
