...
| `powerplan.js` (~200 줄) | 전원 계획 모드 (2 키 토글, 이전 1) — `setPowerPlanMode` / `_initPowerPlanCache` (isPreserved 헬퍼 + 도어 분류 + 휴리스틱 상수 hoist) / `_initOutletOutlines` / `_buildPpVisIdxs` / **`_applyOutletView`** (CL 50983 추출, 미팅 모드 공유) / `[PP]` 어셔션 |
| `meetingmode.js` (~70 줄) | 5/8 미팅 결정사항 모드 (1 키 토글, CL 50983) — `setMeetingMode` 가 `_applyOutletView` 호출. PP 모드와 mutually exclusive. 향후 조명·난방·디딤판 등 시각화 추가 예정. |
...
- 단축키:
  ...
  - **`1`** — **5/8 미팅 결정사항 모드** 토글 (CL 50983): 첫 시각화는 `2` 키 PP 모드와 동일 효과 (가구 hide + 콘센트 outline). 추후 조명·난방 분배기·난방 컨트롤러 위치/수량 시각화 단계적 추가 예정. PP 모드와 mutually exclusive — 한 모드 켜면 다른 모드 자동 종료. 녹색 📋 배지로 PP 의 주황 🔌 배지와 구분.

- **모바일 환경** (CL 50975+, 감지: `'ontouchstart' in window && innerWidth ≤ 1024`):
