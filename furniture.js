...
const FURN_META = {
  ...
  74: { id:74, name:'난방수 분배기',         room:'주방·식당',  pos:{cx:7.29,  cz:1.35 }, size:{W:0.50, D:0.30, H:0.40 }, bbox:[7.14,  1.10,  7.44,  1.60, 0.10, 0.50], meetingOnly:true, source:'@USER 5/8 미팅 결정 — 벽 94 (주방 우벽 x=7.8) 중앙 (z=1.35), 벽에서 앞으로 30cm 당김 (동측면 x=7.44, 벽 내면 7.74 와 30cm 갭, 메인 인입관). 본체 두께 D 15→30cm 두 배 (동측 7.44 고정 + 서측 7.29→7.14 로 -x 확장). 주방 하부장(우) @FURN#52 (x=7.14~7.74, z=0.66~2.64) BEHIND. meetingOnly:true → 평소 숨김, 키 1 미팅 모드에서만 가시.' },
};
...
/* ─────────────────────────────────────────────────────────────────
   거실 가구 시각 숨김 플래그 (사용자 요청, CL 50306+)
   거실 소파(@FURN#70), 다이닝 테이블(@FURN#71), 벤치(@FURN#72) 의 메시
   생성을 건너뜀. FURN_META / FURNITURE / FURNITURE_BBOX 데이터는 그대로
   유지 (어셔션 §M/§U/§CC 영향 없음, SHIFT-aim 라벨은 phantom 표시).
   false 로 변경 시 다시 표시.
   ───────────────────────────────────────────────────────────────── */
const HIDE_LIVING_FURN = true;
...
(function(){
  if (HIDE_LIVING_FURN) return;                // 사용자 요청 시각 숨김
...
(function(){
  ...
  /* ── 치수 (m) ───────────────────────────────────────────────── */
  // 상부장 깊이 0.69 m: 배관 기둥(@ROOM#14, z=0~0.75) 의 앞면과 정렬되도록
  // 사용자 요청 (이전 0.32 m). 표준 상부장보다 깊어 시각적으로 튀어나옴.
  var DL=0.60, DU=0.69;   // 하부장/상부장 깊이
