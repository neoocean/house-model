...
- 단축키: `Space` (모드 토글), `WASD` (이동), `Q`/`E` (위/아래 또는 눈높이), `R` (카메라 리셋), `SHIFT` (조준 라벨+치수), **`1` (5/8 미팅 결정사항 모드, CL 50995 — PP 모드와 mutually exclusive)**, **`2` (전원 계획 모드 토글)**, **`M` (미니맵 표시/숨김)**.
- 모바일 환경 (CL 50979, `ontouchstart` + 화면 ≤ 1024px): UI/미니맵 자동 숨김, 한 손가락 드래그 = 카메라 이동 (WASD 대체), 핀치 = 전진/후진, 짧은 탭 = 문 토글.
